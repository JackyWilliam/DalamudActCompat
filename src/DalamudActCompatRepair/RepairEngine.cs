...
namespace DalamudActCompatRepair
{
    ...
    internal static class RepairEngine
    {
        ...
        internal static string RunningHost()
        {
            foreach (var process in Process.GetProcesses())
            {
                using (process)
                {
                    ...
                    if (name.Equals("DalamudActCompat.Host", StringComparison.OrdinalIgnoreCase)) return name;
                }
            }
            return null;
        }
        ...
        private static void RequireClosed(Func<string> runningProcess)
        {
            ...
            if (name != null) throw new InvalidOperationException("请先在卫月列表停用 DACT，并等待它的 Host 退出。仍在运行：" + name);
        }
        ...
        private static Dictionary<string, object> ReadManifest(string versionDirectory, string version)
        {
            ...
            var manifest = JsonConvert.DeserializeObject<Dictionary<string, object>>(File.ReadAllText(manifestPath));
            ...
            Func<string> runningProcess, Action<string> checkpoint = null)
        {
            ...
            try
            {
                ...
                File.WriteAllText(Within(stage, PluginName + ".json"), JsonConvert.SerializeObject(fresh), new UTF8Encoding(false));
                RequireClosed(runningProcess);
                ...
                ReadManifest(target, TargetVersion);
                log("修复完成：4.4.0.0 → 0.4.4.0。配置未更改，请完整重启一次游戏。");
                log("原安装已备份：" + run);
                if (fresh.ContainsKey("Disabled") && Object.Equals(fresh["Disabled"], true))
                    log("插件原本处于禁用状态；进入游戏后请在插件列表启用 DACT。");
                undo.Clear();
                return new RepairResult { BackupDirectory = run, Message = "修复完成。请完整重启一次游戏，再启用 DACT。" };
            }
