...
public static class ActEncounterMapper
{
    ...
    {
        var combatants = source.Combatants.Select(MapCombatant).ToArray();
        var jobs = combatants
        ...
        return new Encounter(
            source.Id, source.StartTime, source.EndTime, source.ZoneName, source.EnemyName,
            ...
            Array.Empty<DeathEvent>(), Array.Empty<ActionSummary>(), jobs)
        {
            ...
            ParsedPlayers = source.ParsedPlayers?.Select(MapCombatant).ToArray(),
            ParserContext = source.ParserContext,
        };
    }

    private static Combatant MapCombatant(ActCombatantSnapshot combatant) => new(
        combatant.Id,
        combatant.Name,
        combatant.Job,
