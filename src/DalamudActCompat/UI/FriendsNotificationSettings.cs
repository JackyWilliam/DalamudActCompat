...
internal static class FriendsNotificationSettings
{
    internal static bool Draw(PluginConfiguration configuration, UiText text, Action<int>? previewSound = null)
    {
        ...
        DactTheme.TextColored(ControlCenterWindow.Gold, text.Get("来信气泡", "Incoming message bubbles"));
        var enabled = configuration.FriendNotificationsEnabled;
        var changed = DactTheme.Checkbox(text.Get("显示来信气泡", "Show incoming message bubbles"), ref enabled);
        ...
        if (DactTheme.Checkbox(text.Get("播放新消息提示音（独立于气泡）", "Play message sound (independent of bubbles)"), ref sound))
