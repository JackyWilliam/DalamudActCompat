...
“游戏区域”默认按当前 Dalamud 的启动器目录自动检测：`XIVLauncherCN` 使用国服网络协议，`XIVLauncher` 使用国际服网络协议，不根据客户端语言判断，因此国际服中文语言包不会被误判。无法识别自定义目录时暂按国际服处理，并可手动覆盖；切换后插件会刷新正在运行的解析器与扩展 Host，而动作名和日志语言仍跟随游戏客户端。FFLogs 区间估算会同步使用国服 CN 分区或国际服最新全球分区。
