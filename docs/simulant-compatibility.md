...
开始按钮现在要求初始化、防火墙和模拟区域均就绪，阶段切换已结束，且游戏当前地图与预设一致。原版只凭已选预设即可启动，曾在进入副本前或返回旅店后仍显示“模拟已开始”。不满足条件时保留当前会话并提示原因，不自动切换地图；加载区域时新增请求日志，以核对用户报告的地图延迟。日志中已有一次进入 1122 / 返回 177，但用户反馈曾点击加载而画面没有及时切换，其原因尚未确定；实际实体生成与机制效果仍待复测。
