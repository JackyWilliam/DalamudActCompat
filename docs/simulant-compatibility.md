...
绝伊甸 P5 地火曾在生成 EObj 后立即查询实体时报找不到对象，导致整条时间轴中断；只读检查确认游戏中已有 BaseId 2014199 的场景物件。Host 实体增量约每 30 毫秒同步，无法满足原版生成后立刻回查的要求。原生扫描完成后，Simulant 的实体枚举改用原版 GameObjectManager / GameObjectPtrs 直接读取即时列表，保留自身及 PostNamazu 的权限检查；扫描前仍使用 Triggernometry 原有列表。回归通过原版 DLL 和托管内存夹具验证无 IPC 更新时新 EObj 可立即查询、撤销权限后不读取。修订后的地火画面仍待实机复测。
