...
右键好友行选择“设置备注…”或“修改备注…”，最多 40 个 Unicode 码点，单行，保存空文本或点击“清除备注”可恢复账号名。列表、聊天标题、消息作者和提醒显示“备注（账号名）”，账号身份、管理员/赞助身份和消息内容不变。

备注属于当前登录账号，直接保存到好友服务，不依赖配置备份上传。`PUT friends/{relationshipId}/remark` 接收 `{text,revision}` 并返回已保存的 `{text,revision}`；好友列表中的 `remark` 只包含当前查看者自己的值。同账号其他设备随现有列表轮询同步（通常约 5 秒）；对方和其他账号看不到备注。解除关系会删除双方各自的备注，重新添加从空值开始。

编辑器保留打开时的版本，其他设备修改后返回 409，不静默覆盖。相同内容重试安全；请求结果未知时保留草稿并刷新服务器值。切换账号丢弃旧快照和编辑状态，旧请求不能使用新账号凭据。旧服务未返回 `remark` 时禁用备注保存，好友、聊天和配置备份继续使用原有接口。本机聊天加密状态格式保持不变。
