...
本阶段提供 DACT / 后续宠物插件共用的账号、好友、在线状态与聊天契约，以及管理端官方消息的接收类型。完整好友抽屉、聊天气泡、宠物 AI 和支付不在本阶段。现有“好友邀请”仍是注册激活码邀请，与新增好友申请分开。
...
- GET 和心跳不会消耗离线消息。后台连接不替尚未实现的聊天界面自动 ACK。
- `DeliveredAt` 代表进入可用历史的时间，不是已读回执。

未来界面应以新快照替换旧内容，避免不断追加造成客户端无限保留。同步多客户端时按消息 ID 去重。ACK 最多包含 3 个发给自己的 ID；重复 ACK 安全。若消息在读取与确认之间已被裁剪，返回的 `RetiredIds` 表示它已不再保留，必须以返回快照校正本地状态，不能重新插入。未读/已读、通知频率和跨设备本地缓存策略留待界面阶段。
...
发布建议先服务端后客户端；反向升级已做兼容验证。当前只是本地候选实现，不等于已发布或已安装游戏。后续完整 UI 接入时需要补游戏内抽屉/气泡、通知、实际多设备体验及宠物共享账号消费者验收。
