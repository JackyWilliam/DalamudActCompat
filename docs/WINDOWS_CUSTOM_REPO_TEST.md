...
```powershell
gh workflow run Release -f tag=v0.3.10.0 --repo JackyWilliam/DalamudActCompat
```
...
```powershell
./tools/update-pluginmaster.ps1 -Version 0.3.10.0 -Changelog "Add split resource packs and the 0.3.10.0 combat-meter features."
...
git commit -m "chore: 更新 Dalamud 插件仓库 0.3.10.0"
git push
```
...
```powershell
powershell -ExecutionPolicy Bypass -File .\tools\collect-release.ps1 -ExpectedAssemblyVersion 0.3.10.5 -ExpectedDalamudApiLevel 15
```
