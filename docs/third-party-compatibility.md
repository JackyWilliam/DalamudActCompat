...
| --- | --- | --- | --- |
| Cactbot | [OverlayPlugin/cactbot](https://github.com/OverlayPlugin/cactbot) | 官方 Release ZIP；OverlayPlugin addon，包含 DLL、UI、资源与时间轴 | 可独立安装完整资源；内置 Cactbot 事件源；HTML Overlay 按 OverlayPlugin 锁定/穿透语义接入宿主级移动、缩放与官方状态事件，等待游戏内联动验收 |
...
| ACT.FoxTTS | [Noisyfox/ACT.FoxTTS](https://github.com/Noisyfox/ACT.FoxTTS) | 当前官方 Release 为 7z，解压后选择 `ACT.FoxTTS.dll`；普通 ACT 插件 | 可安装和加载；ACT `PlayTtsMethod` 与 FoxTTS `Speak` 已桥接，音频设备和所选网络语音后端等待游戏内实测 |
